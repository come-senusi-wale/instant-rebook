...
import TestModel from "../../database/models/costomer.model";
...
export const getSingleUserController = async (
  ...
  try {
    ...
    // 2️⃣ Check if user started test
    const userTest = await RoomModel.findOne({
      hotel: user._id,
    });
...
export const getAllUserController = async (
  ...
  try {
    ...
    const users = await HotelModel.find(query).skip(skip).limit(limitCheck).sort({createdAt: -1})
...
export const messageSingleUserController = async (
  ...
  try {
    ...
    const user = await HotelModel.findOne({ _id: userId });
